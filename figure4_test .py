...
# -*- coding: utf-8 -*-
"""
Created on Tue Jul 12 13:40:51 2022
...
for x in time_list:
    ...
    for result_x in range(test.lattice_length):
        for result_y in range(test.lattice_length):
           ...
           # counter[time_list[int(x)]][optimal_at_x] =+1 
            ...
            ax[0,1].set( ylim = (0,100), xlim =  (0,100))
            ax[0,1].set_xticks([], [])
            # pad = -.05
    ...
    im = ax[0,0].imshow(result, cmap=plt.get_cmap('hot'), interpolation='nearest',
                   vmin=0, vmax=1)
    ax[0,0].set( ylabel='y length', ylim = (0,100), xlim =  (0,100))
    ax[0,0].set_xticks([], [])
    # save frame



time_list  = np.linspace(20,100,1)
counter = np.zeros((np.size(time_list),16))
...
for x in time_list:
    ...
    for result_x in range(test.lattice_length):
        for result_y in range(test.lattice_length):
           ...
           # counter[time_list[int(x)]][optimal_at_x] =+1 
            ...
            ax[1,1].set(ylim = (0,100), xlim =  (0,100))
            ax[1,1].set_xticks([], [])
            # pad = -.05
            # pos = ax1.get_position()
            # pos.y0 = pos.y0+pad
    ...
    im = ax[1,0].imshow(result, cmap=plt.get_cmap('hot'), interpolation='nearest',
                   vmin=0, vmax=1)
    ax[1,0].set(ylabel='y length', ylim = (0,100), xlim =  (0,100))
    ax[1,0].set_xticks([], [])
    # save frame
...
time_list  = time_list  = np.linspace(80,100,1)
counter = np.zeros((np.size(time_list),16))
    ...
    for result_x in range(test.lattice_length):
        for result_y in range(test.lattice_length):
           ...
           # counter[time_list[int(x)]][optimal_at_x] =+1 
            ...
            ax[2,1].set(xlabel='x length', ylim = (0,100), xlim =  (0,100))
            # pad = -.05
            # pos = ax1.get_position()
            # pos.y0 = pos.y0+pad
            # pos.y1 = pos.y1+pad
