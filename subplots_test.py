...
for x in time_list:
    ...
    for result_x in range(test.lattice_length):
        for result_y in range(test.lattice_length):
           ...
           # counter[time_list[int(x)]][optimal_at_x] =+1 
            ...
            im = ax1.scatter(result_y,result_x, color=color_matrix[result_x,result_y])
            ax1.set(xlabel='x length', ylabel='y length', ylim = (0,100), xlim =  (0,100),
    ...
    im = ax0.imshow(result, cmap=plt.get_cmap('hot'), interpolation='nearest',
                   vmin=0, vmax=1)
    ax0.set(xlabel='x length', ylabel='y length', ylim = (0,100), xlim =  (0,100),
                   title='Steady State Drug Diffusion w/Constant Source & Reaction')
...
with imageio.get_writer('subplot_test2.gif', mode='I') as writer:
